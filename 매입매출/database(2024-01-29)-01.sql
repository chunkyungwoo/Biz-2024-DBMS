...
-- 알수 없다.
-- 이럴때 테이블간에 JOIN을 실행하여 확인을 해야한다.

-- Table 을 JOIN 을 통해 "참조무결성"검증 절차가 필요하다
-- 상품과 매입매출 테이블의 Relation 검증
-- 결과가 하나라도 출력이 되면
-- tbl_iolist 에는 상품코드 데이터가 포함되어 있지만
-- tbl_products 에는 상품코드가 누락되어있다
-- 참조관계(참조무결성)이 문제가있다 라는 것이다
